    ...
    def __tokenizeText(self, text):
        ...
        for sent in taggedSents:
            for taggedToken in sent:
                ...
                if len(token) > 2 and token not in stopwords.words('english') and containsAlpha(token):
                    pos = getWordNetPos(taggedToken[1])
                    if pos is not None:
                        normToken = lmtzr.lemmatize(token, pos)
                        if len(normToken) < 3 or (len(normToken) < 4 and containsPun(token)): continue 
                        tokens.append(normToken)
